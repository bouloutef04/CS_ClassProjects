...
public class project3_GUI extends JFrame{

    public JFrame window;// Main window
    private JPanel upper;// Upper section
    private JPanel lower;// Lower section

    String[] dbProportiesItems = { "project3.properties", "bikedb.properties", "modeldb.properties" };
    String[] userProportiesItems = { "root.properties", "client1.properties", "client2.properties",
            "newuser.properties", "mysteryuser.properties" };
...
//        		resultTable.setModel(DbUtils.resultSetToTableModel(DbUtils.resultSetToel(resultSet)));
        		...
        		while(resultSet.next()) {
        			...
        			for(i = 0; i < numberOfColumns; i++) {
        				...
        				resultTable.setValueAt(resultSet.getString(columnNames[i]), j, i);
        		...
        		//Update the count
        		updateOperationsLog("num_queries");
    ...
    public int setUpdate(String update, Statement statement) {
    	try {
	    	...
	    	updateOperationsLog("num_updates");
	    	return affected;
    	} catch (SQLException e){
    		JOptionPane.showMessageDialog(null, e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
    	}
    ...
    public void updateOperationsLog(String columnsName) throws SQLException {
    	
    	//Connect to the operationslog db

    	FileInputStream filein;
		try {
			filein = new FileInputStream("operationslog.properties");
		
	    	MysqlDataSource oDataSource = new MysqlDataSource();
	    	Properties properties = new Properties();
	    
			properties.load(filein);
	    	
	    	oDataSource.setURL(properties.getProperty("MYSQL_DB_URL"));
	    	oDataSource.setUser(properties.getProperty("MYSQL_DB_USER"));
	    	oDataSource.setPassword(properties.getProperty("MYSQL_DB_PASSWORD"));
	    	oConnect = oDataSource.getConnection();
	    	
	    	//Set an update for the count and close
	    	System.out.println("UPDATE operationscount SET " + columnsName +  " = " + columnsName + " 1 WHERE login_username = " + userProp + ";");
			String oCommand = String.format("UPDATE operationscount SET %s = %s + 1 WHERE login_username = '%s';", columnsName, columnsName, userProp);
			oConnect.createStatement().executeUpdate(oCommand);
			oConnect.close();
...
//        lower.add(resultTable, BorderLayout.CENTER);
	...
	public static void main(String[] args) {
        project3_GUI projectGeneral = new project3_GUI();
